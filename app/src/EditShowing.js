...
class EditShowing extends React.Component {
  ...
  editShowingInShowingList(films) {
    ...
    if (textInput.length === 0) {
      ...
      Date.parse(dateInput) < Date.parse(this.props.location.state.todayDate)
    ) {
      this.setState({
        invalidDate: true,
      });
      ...
      if (updatedFilmWithoutThisShow.id === this.props.location.state.filmId) {
        ...
        console.log(this.state.showingId);
    ...
    return (
      <div className="EditShowing">
        <div>
          ...
          />
          <Error error={invalidFilmName} info="Nie ma takiego filmu!" />
          <Error
            error={emptyCinemaHallField}
            info="Pole sala nie może być puste!"
          />
          <Error error={invalidCinemaHallNumber} info="Nie ma takiej sali!" />
          <Error error={emptyDateField} info="Pole data nie może być puste!" />
